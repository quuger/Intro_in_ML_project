from topic_segmenter.segmenter.Window import Window
from topic_segmenter.model.SimilarTopicCalculator import SimilarTopicCalculator
from topic_segmenter.model.ReplyObjectPredictor import ReplyObjectPredictor
from topic_segmenter.text.Topic import Topic
...
class ConversationSegmenter:
    ...
    def segment(self):
        ...
        while i < len(topicSet):
            ...
            if (
                messages[0].getID() > otherMessages[0].getID()
                and messages[-1].getID() < otherMessages[-1].getID()
            ):
